...
% 1. load data
[X,Y] = readData('mnist','train');
sz = size(X);
X = reshape(X, prod(sz(1:3)), sz(4)); % convert 4-D array to 2-D array 
X = normalize(X,2);
...
trainingOptions = struct(...
    ...
    'GradientThreshold', 1,...
    'UseGPU', false, ... % currently GPU is slower than CPU
    'Shuffle', true, ...
    'Verbose', true, ...
...
Net = {... % Note: this network doesn't work well for MNIST
    FullyConnectedLayer(InputLength,50),...
    ReluLayer(),...
    FullyConnectedLayer(50,100),...
    ReluLayer(),...
    FullyConnectedLayer(100,200),...
    ReluLayer(),...
    FullyConnectedLayer(200,OutputLength),...
    MSERegressionLayer() };
